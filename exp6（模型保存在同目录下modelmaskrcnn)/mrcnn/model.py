...
def fpn_classifier_graph(rois, feature_maps, image_meta,
    ...
    #mrcnn_bbox = KL.Reshape((rois.shape[1], num_classes, 4), name="mrcnn_bbox")(x)
    #mrcnn_bbox = KL.Reshape((tf.shape(rois)[1], num_classes, 4), name="mrcnn_bbox")(x)
    mrcnn_bbox = KL.Reshape((1000, num_classes, 4), name="mrcnn_bbox")(x)
    print("Shape of x after dense layer:", x.shape)
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
           ...
           # anchors = KL.Lambda(lambda x: tf.Variable(anchors), name="anchors")(input_image) 直接使用一个普通张量来生成锚点。以下代码展示了如何替换 Lambda 层，确保不会创建不受 Layer 管理的变量。
            # 替换原始的 Lambda 生成 anchors 的代码
            anchors_tensor = tf.constant(anchors, dtype=tf.float32)
            anchors_tensor = tf.expand_dims(anchors_tensor, axis=0)  # 增加一个维度，使其与批次对齐
            anchors_output = tf.tile(anchors_tensor, [tf.shape(input_image)[0], 1, 1, 1])
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        # Generate a unique checkpoint path with timestamp
        import datetime
        timestamp = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
        checkpoint_path = f"C:/Users/Administrator/Desktop/heangcomputervision/modelmaskrcnn/checkpoint_{timestamp}_epoch_{{epoch:02d}}.h5"
